...
If need be you might need to configure your drives to use 64B metadata. For that:
```
...
Configure the drives by running `sudo HUGEMEM=<size> scripts/setup.sh` (can be reversed by `sudo HUGEMEM=<size> CLEAR_HUGE=yes scripts/setup.sh reset`). We used size of 55000 for remote and 23000 for local. Note you might also need to change the runtime env setup (`*.conf`) for the correct PCIe address of your disk.
